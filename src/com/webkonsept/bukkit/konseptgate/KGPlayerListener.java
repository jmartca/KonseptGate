...
public class KGPlayerListener implements Listener {
    ...
    public void onPlayerInteract(final PlayerInteractEvent event){
        ...
        if (event.getAction().equals(Action.PHYSICAL) && event.getClickedBlock().getType().equals(Material.STONE_PRESSURE_PLATE)){
                    ...
                    if (plugin.permit(player, "konseptgate.teleport")){
                        ...
                        if (plugin.gates.gateName.containsKey(origin.getTargetName())){
                            ...
                            if (destination != null){
                                ...
                                if (origin.getCommand().isEmpty()){
                                    plugin.verbose("Gate "+origin.getName()+" has no associated command");
                                }
                                else {
                                    plugin.verbose("Gate "+origin.getName()+" has command '"+origin.getCommand()+"' tied to it");
                                    player.performCommand(origin.getCommand());
                                }
        ...
        else if (event.getAction().equals(Action.RIGHT_CLICK_BLOCK) && event.getClickedBlock().getType().equals(Material.STONE_PRESSURE_PLATE)){
